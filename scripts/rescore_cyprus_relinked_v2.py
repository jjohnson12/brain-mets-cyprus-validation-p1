...
Example:
  python rescore_cyprus_relinked_v2.py \
    --dataset /path/to/analysis_workspace/dataset_relinked.csv \
    --out-dir /path/to/analysis_workspace/rescore_relinked
"""
...
def main():
    ...
    for idx, row in df.loc[df["GTMaskPath"].notna() & (df["GTMaskPath"].astype(str).str.strip() != "")].iterrows():
        ...
        # In archived analysis workspaces, model_ensemble-label.nii.gz files may have
