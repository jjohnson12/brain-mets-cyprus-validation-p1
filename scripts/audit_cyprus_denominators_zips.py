...
compares them with the final manuscript targets (186 T1-post studies, 170
paired reference masks, 169 tumor-core-positive studies, and 125 consecutive
available-reference pairs).

Example:
  python audit_cyprus_denominators_zips.py \
    --zip-root /path/to/cyprus_archives \
    --out-dir /path/to/results/denominator_audit
...
def scan(zip_root: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    summary: dict[str, Any] = {
        ...
        "reference_pairs_compressed_eligible_sequence": ref_pairs_comp,
        ...
        "manuscript_targets": {
            "t1c_studies": 186,
            "paired_reference_masks": 170,
            "tumor_core_positive_studies": 169,
            "longitudinal_case_units": 45,
            "consecutive_available_reference_pairs": 125,
            "strictly_adjacent_reference_pairs": 116,
        },
    }
    return rows, summary
...
def write_outputs(rows: list[dict[str, Any]], summary: dict[str, Any], out_dir: Path):
    ...
    keys = [
        ...
        "reference_pairs_compressed_eligible_sequence",
    ...
    lines.append(f"  T1-post studies == 186: {summary['t1c_timepoints'] == 186}")
    lines.append(f"  paired T1-post/reference masks == 170: {summary['paired_t1c_mask_timepoints'] == 170}")
    lines.append(f"  tumor-core-positive studies == 169: {summary['tumor_core_positive_timepoints'] == 169}")
    lines.append(f"  longitudinal case units == 45: {summary['case_units_with_2plus_reference_masks'] == 45}")
    lines.append(f"  available-reference pairs == 125: {summary['reference_pairs_compressed_eligible_sequence'] == 125}")
    lines.append(f"  strictly adjacent reference pairs == 116: {summary['reference_pairs_original_adjacent'] == 116}")
